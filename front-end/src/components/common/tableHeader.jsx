...
const TableHeader = ({ sortColumn, onSort }) => { 
    ...
    return <div className='row fs-2 fw-bold text-white' style={{ letterSpacing: '0.1em' }}>
        <div onClick={ () => raiseSort('title') } className={ `${styles.taskCol} ps-4 text-hover-primary cursorPointer` }>
            TASK{ renderSortIcon('title') }
        </div>
        <div onClick={ () => raiseSort('description') } className={ `${styles.descriptionCol} cursorPointer` }>
            DESCRIPTION{ renderSortIcon('description') }
        </div>
        <div onClick={ () => raiseSort('type.name') } className={ `${styles.typeCol} cursorPointer` }>
